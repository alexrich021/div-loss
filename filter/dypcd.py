...
def get_camera_depth_information(filename):
    with open(filename) as f:
        lines = f.readlines()
        lines = [line.rstrip() for line in lines]
    ...
    for ref_view, src_views in pair_data:
        ...
        for src_view in src_views:
            ...
            masks, geo_mask, depth_reprojected, x2d_src, y2d_src = check_geometric_consistency(
                args, ref_depth_est, ref_intrinsics, ref_extrinsics, src_depth_est, src_intrinsics, src_extrinsics)
        ...
        if args.endrange_filter:
            depth_min, depth_interval = get_camera_depth_information(
                os.path.join(args.testpath, args.split, os.path.basename(scan_folder),
                             'cams', f'{ref_view:08}_cam.txt'))
            depth_interval *= 1.06
            depth_max = depth_min + depth_interval * args.numdepth
        ...
        valid_points = final_filter_mask if args.endrange_filter else final_mask
...
def dypcd_filter_worker(args, scene):
    ...
    if scene in tank_cfg.scenes:
        ...
        args.endrange_filter = scene_cfg.endrange_filter

    filter_depth(args, pair_folder, scan_folder, out_folder, scan_ply_path)
