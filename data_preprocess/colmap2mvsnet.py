...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_ext', default='.bin', choices=['.txt', '.bin'])
    ...
    cam_dir = os.path.join(args.out_dir, 'cams')
    renamed_dir = os.path.join(args.out_dir, 'images')
    ...
    print('intrinsic[1]\n', intrinsic[list(intrinsic.keys())[0]], end='\n\n')
    ...
    print('extrinsic[1]\n', extrinsic[list(extrinsic.keys())[0]], end='\n\n')
    ...
    print('depth_ranges[1]\n', depth_ranges[list(depth_ranges.keys())[0]], end='\n\n')
    ...
    def calc_score(inputs):
        i, j = inputs
        image_id_i, image_id_j = image_ids[i], image_ids[j]
        id_i = images[image_id_i].point3D_ids
        id_j = images[image_id_j].point3D_ids
        id_intersect = [it for it in id_i if it in id_j and it != -1]
        cam_center_i = -np.matmul(extrinsic[image_id_i][:3, :3].transpose(), extrinsic[image_id_i][:3, 3:4])[:, 0]
        cam_center_j = -np.matmul(extrinsic[image_id_j][:3, :3].transpose(), extrinsic[image_id_j][:3, 3:4])[:, 0]
        score = 0
    ...
    for i in range(num_images):
        ...
        with open(os.path.join(cam_dir, '%08d_cam.txt' % i), 'w') as f:
            ...
            for j in range(3):
                ...
                f.write('\n')
            f.write('\n%f %f %f %f\n' % (depth_ranges[image_id][0], depth_ranges[image_id][1],
                                         depth_ranges[image_id][2], depth_ranges[image_id][3]))
    with open(os.path.join(args.out_dir, 'pair.txt'), 'w') as f:
        f.write('%d\n' % len(images))
        for i, sorted_score in enumerate(view_sel):
            f.write('%d\n%d ' % (i, len(sorted_score)))
            for k, s in sorted_score:
                f.write('%d %f ' % (k, s))
            f.write('\n')
    ...
    for i in range(num_images):
        ...
        if args.convert_format:
            ...
            shutil.copyfile(os.path.join(image_dir, images[image_id].name), os.path.join(renamed_dir, '%08d.jpg' % i))
