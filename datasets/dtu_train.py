...
class MVSDataset(Dataset):
    def __init__(self, datapath, listfile, mode, nviews, ndepths=192, interval_scale=1.06, random_view=False,
                 sup_view_selection='topk', **kwargs):
        ...
        self.sup_view_selection = sup_view_selection
        ...
        for scan in scans:
            ...
            with open(os.path.join(self.datapath, pair_file)) as f:
                ...
                for view_idx in range(num_viewpoint):
                    ...
                    view_info = f.readline().rstrip().split()
                    src_views = [int(x) for x in view_info[1::2]]
                    scores = [float(x) for x in view_info[2::2]]
                    # light conditions 0-6
                    # light conditions 0-6 for training
                    # light condition 3 for testing (the brightest?)
                    for light_idx in range(7):
                        metas.append((scan, light_idx, ref_view, src_views, scores))
    ...
    def __getitem__(self, idx):
        sample = {}
        meta = self.metas[idx]
        scan, light_idx, ref_view, src_views, scores = meta
        ...
        if self.sup_view_selection == 'topk':
            sup_view_ids = [ref_view] + src_views[:self.nviews - 1]
        elif self.sup_view_selection == 'score':
            score_weights = scores / np.sum(scores)
            src_idx = sorted(np.random.choice(np.arange(len(src_views)), self.nviews - 1, False, p=score_weights))
            sup_view_ids = [ref_view] + [src_views[s_idx] for s_idx in src_idx]
        else:
        ...
        sup_imgs = []
        sup_proj_matrices = []
        for i, vid in enumerate(sup_view_ids):
            # NOTE that the id in image file names is from 1 to 49 (not 0~48)
            img_filename = os.path.join(self.datapath, 'Rectified/{}_train/rect_{:0>3}_{}_r5000.png'.format(scan, vid + 1, light_idx))
            proj_mat_filename = os.path.join(self.datapath, 'Cameras/train/{:0>8}_cam.txt').format(vid)

            sup_img = cv2.cvtColor(cv2.imread(img_filename), cv2.COLOR_BGR2RGB)
            intrinsics, extrinsics, depth_min, depth_interval, _ = self.read_cam_file(proj_mat_filename)
            ...
            sup_proj_matrices.append(proj_mat)
            sup_imgs.append(sup_img)
        sup_imgs = self.center_images(np.stack(sup_imgs, axis=0)).transpose([0, 3, 1, 2])
        ...
        # ms proj_mats
        sup_proj_matrices = np.stack(sup_proj_matrices)
        stage2_sup_pjmats = sup_proj_matrices.copy()
        stage2_sup_pjmats[:, 1, :2, :] = sup_proj_matrices[:, 1, :2, :] * 2
        stage3_sup_pjmats = sup_proj_matrices.copy()
        stage3_sup_pjmats[:, 1, :2, :] = sup_proj_matrices[:, 1, :2, :] * 4

        ctr_proj_matrices_ms = {
            "stage1": sup_proj_matrices,
            "stage2": stage2_sup_pjmats,
            "stage3": stage3_sup_pjmats
        }
        ...
        sample["mask"] = mask
        sample["sup_imgs"] = sup_imgs
        sample['sup_proj_matrices'] = ctr_proj_matrices_ms
