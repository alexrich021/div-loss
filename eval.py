...
from losses.modules import WeightNet
from losses.homography import inverse_warping
...
from filter.gipuma import gipuma_filter
...
from filter import depth_render
...
parser.add_argument('--loadckpt', default='./outputs/DIV-MVS/model_000014.ckpt', help='load a specific checkpoint')
parser.add_argument('--outdir', default='./outputs/DIV-MVS/dtu', help='output dir')
...
parser.add_argument('--disp_threshold', type=float, default=0.01, help='gipuma thresh')
parser.add_argument('--fusibile_exe_path', type=str)
parser.add_argument('--mask_using_gt', action='store_true')


parser.add_argument('--no_test',action='store_true',help='fusion without testing')
parser.add_argument('--filter_method', type=str, choices=['normal', 'gipuma', 'none'], default='normal')
# os environment
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(TestImgLoader):
            ...
            for filename, cam, img, depth_est, photometric_confidence in zip(filenames, cams, imgs, \
                ...
                depth_color = visualize_depth_2(depth_est)
                ...
                cv2.imwrite(confidence_img_filename, visualize_depth_2(photometric_confidence))
...
def filter_depth(pair_folder, scan_folder, out_folder, plyfilename, prob_threshold, num_consistent, img_dist_thresh, depth_thresh):
    ...
    if args.mask_using_gt:
        h, w = read_img(os.path.join(scan_folder, 'images', f'{pair_data[0][0]:08}.jpg')).shape[:2]
        renderer = depth_render.DepthRenderer(os.path.join(pair_folder, 'mesh_aligned_0.05.ply'), h, w)
    ...
    for i, (ref_view, src_views) in enumerate(pair_data):
        ...
        if args.mask_using_gt:
            gt_depth = renderer(ref_intrinsics, ref_extrinsics)
            gt_mask = gt_depth != 0.
            final_mask = np.logical_and(final_mask, gt_mask)
            save_mask(os.path.join(out_folder, "mask/{:0>8}_gt.png".format(ref_view)), gt_mask)
        ...
        if i != 0 and (i % 500) == 0:  # save intermediate pcd to ease memory
            ...
            intermediate_filename = plyfilename.replace('.ply', f'_{int(i // 500)}.ply')
            PlyData([el]).write(intermediate_filename)
            intermediate_filenames.append(intermediate_filename)
            print("saving an intermediate model to", intermediate_filename)
    ...
    for intermediate_filename in intermediate_filenames:
        intermediate_pcd = o3d.io.read_point_cloud(intermediate_filename)
        vs = np.array([tuple(v) for v in np.asarray(intermediate_pcd.points)],
                      dtype=[('x', 'f4'), ('y', 'f4'), ('z', 'f4')])
        cs = np.array([tuple(v) for v in (np.asarray(intermediate_pcd.colors) * 255).astype(np.uint8)],
                      dtype=[('red', 'u1'), ('green', 'u1'), ('blue', 'u1')])
        vertexs = np.concatenate((vertexs, vs), axis=0)
        vertex_colors = np.concatenate((vertex_colors, cs), axis=0)
...
def pcd_filter_worker(scan):
    if 'dtu' in args.testlist:   # dtu
        scan_id = int(scan[4:])
        save_name = 'mvsnet{:0>3}_l3.ply'.format(scan_id)
    else:
        scan_id = 0
        save_name = '{}.ply'.format(scan)
    ply_filename = os.path.join(args.outdir, save_name)
    ...
    filter_depth(pair_folder, scan_folder, out_folder, ply_filename,
                 prob_threshold=args.prob_thres, num_consistent=args.num_consistency,
                 img_dist_thresh=img_dist_thres.get(str(scan_id), args.img_dist_thres), depth_thresh=args.depth_thres)
...
if __name__ == '__main__':
    ...
    if args.filter_method == 'normal':
        ...
        gipuma_filter(testlist, args.outdir, args.prob_thres, args.disp_threshold, args.num_consistency,
