...
def homo_warping(src_fea, src_proj, ref_proj, depth_values):
    ...
    with torch.no_grad():
        ...
        proj_z = torch.clip(proj_xyz[:, 2:3, :, :], 1e-7)
        ...
        mask = (proj_xyz[:, 2:3, :, :] < 1e-7)\
            .view(batch, 1, num_depth, height, width)\
            .expand(batch, channels, num_depth, height, width)

    warped_src_fea = F.grid_sample(src_fea, grid.view(batch, num_depth * height, width, 2), mode='bilinear',
                                   padding_mode='zeros', align_corners=True)
    warped_src_fea = warped_src_fea.view(batch, channels, num_depth, height, width)
    warped_src_fea[mask] = 0.
...
class FeatureNet(nn.Module):
    def __init__(self, base_channels, num_stage=3, stride=4, arch_mode="unet", use_dino=False):
...
class CostRegNet(nn.Module):
    ...
    def forward(self, x, return_vol_feature=False):
        ...
        p = self.prob(x)#2,1,48,128,160
        if return_vol_feature:
            return p, x
        return p
